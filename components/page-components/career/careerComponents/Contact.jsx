...
function Contact() {
  const [open, setOpen] = useCarrerContext();
  return (
    <ClientPortal selector="#portal-carrer">
      <AnimateSharedLayout>
        <AnimatePresence exitBeforeEnter>
          {open && (
            ...
            >
              <motion.div
                variants={overlay}
                className="absolute -top-4 left-0 w-[120%] h-[120%] cursor-pointer bg-darkblue/75 blurry hover:bg-darkblue/50 transition-all"
                onClick={() => {
                  setOpen(false);
                }}
              ></motion.div>
              <motion.div
                variants={item}
                layout
                className="h-auto     scrollbar-styling rounded-xl relative z-[4000]"
              >
